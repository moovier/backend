...
def normalize_ratings(ratings):
    ...
    def normalizer(x): return (x - min_rating) / (max_rating - min_rating)
...
def recommend_movies(model, movies, ratings, movies_to_indices, indices_to_movies, users_to_indices, top_k):
    ...
    for user_id in ratings['userId'].unique():
        ...
        recommended_movies = [
            indices_to_movies.get(movies_not_watched[rating][0]) for rating in ratings_sorted_indices
        ]
        recommended_movies = movies[movies['movieId'].isin(recommended_movies)]['title']

        recommended_movies_for_all_users.append({'userId': user_id, 'recommendations': '|'.join(recommended_movies)})
